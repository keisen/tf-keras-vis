...
def test__call__if_seed_input_shape_is_invalid(model):
    scorecam = ScoreCAM(model)
    with pytest.raises(ValueError):
        scorecam(CategoricalScore(1, 2), np.random.sample((8, )), max_N=3)


def test__call__if_seed_input_has_not_batch_dim(model):
    scorecam = ScoreCAM(model)
    result = scorecam(CategoricalScore(1, 2), np.random.sample((8, 8, 3)), max_N=3)
    assert result.shape == (1, 8, 8)


def test__call__(model):
    scorecam = ScoreCAM(model)
    result = scorecam(CategoricalScore(1, 2), np.random.sample((1, 8, 8, 3)), max_N=3)
    assert result.shape == (1, 8, 8)
...
def test__call__if_model_has_multiple_inputs(multiple_inputs_model):
    ...
    result = scorecam(CategoricalScore(
